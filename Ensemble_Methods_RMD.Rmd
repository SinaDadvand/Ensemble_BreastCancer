...
```{r Leave-1-Out Cross Validation Model}
# 
# ans <- numeric(length(BC.train[,1]))
# for (i in 1:length(BC.train[,1])) {
#   mytree <- rpart(Class ~ ., BC.train[-i,])
#   mytree.pred <- predict(mytree,BC.train[i,],type="class")
#   ans[i] <- mytree.pred
# }
# LOOCV.pred <- factor(ans,labels=levels(BC.train$Class))
# table(LOOCV.pred,BC.train$Class)
# 
# LOOCV_acc = round(acc_calculation(predicted = LOOCV.pred, actual = BC.train$Class),4)
```
